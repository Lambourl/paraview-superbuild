...
ParaView version 6.1.0, checkout the `v6.1.0` tag of ParaView and
superbuild.
...
To checkout a specific tag from the superbuild git repository:
    ...
    $ git checkout v6.1.0 # replace `v6.1.0` with tag name of your choice
...
version.  For example, you could use superbuild `v6.1.0`, to build the
...
control how superbuild finds its source packages:
 ...
 1. If you want to use git to checkout ParaView source (default), then set
    ...
    repository. Use `master` for latest development code, `v6.1.0` for the
    6.1.0 release, `release` for latest stable release, or a specific ParaView
 ...
 3. Another option is to use a source tarball of a ParaView release. For that,
    set `paraview_SOURCE_SELECTION` to the version to build such as `6.1.0`.
...
The following flags affect ParaView directly:
  ...
  * `paraview_SOURCE_SELECTION` (default `6.1.0`): The source to use for
